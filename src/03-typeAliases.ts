...
function getBuddyListFriends(list: BuddyList): Friend[] {
  return list.members.filter(
    (buddy): buddy is Friend => "phone" in buddy
  );
}

console.log("Football friends:", getBuddyListFriends(myFootballBuddies).map(f => f.name));
console.log("Band buddies: ", getBuddyListFriends(myBandBuddies).map(f => f.name))
